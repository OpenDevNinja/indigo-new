...
const Countries = () => {
 ...
 const handleAddClick = () => {
  ...
  const handleEditClick = (country) => {
    ...
    setFormValues({ name: country.name });
    setFormErrors({});
    setIsModalOpen(true);
  };
  ...
  const validateForm = () => {
    ...
    if (!formValues.name.trim()) {
      errors.name = "Le nom du pays est requis";
    }
  ...
  const handleFormSubmit = async (e) => {
    ...
    try {
      if (selectedCountry) {
        ...
        await CountryService.update(selectedCountry.id, formValues);
        toast.success(`Pays "${formValues.name}" mis à jour avec succès`);
      } else {
        // Création d'un nouveau pays
        await CountryService.create(formValues);
        toast.success(`Pays "${formValues.name}" ajouté avec succès`);
      }
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      await CountryService.delete(selectedCountry.id);
      toast.success(`Pays "${selectedCountry.name}" supprimé avec succès`);
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      ...
      <h1 className="text-2xl font-bold mb-6 text-gray-800 dark:text-white">Gestion des Pays</h1>
      ...
      <ModalForm
        ...
        title={selectedCountry ? `Modifier le pays: ${selectedCountry.name}` : "Ajouter un nouveau pays"}
      >
          ...
          <div className="flex justify-end space-x-3 mt-6">
            ...
            >
              {formSubmitting ? (
                ...
                selectedCountry ? "Mettre à jour" : "Ajouter"
              )}
            </button>
          </div>
        </form>
      ...
      <DeleteAlert
        ...
        itemName={selectedCountry ? `le pays "${selectedCountry.name}"` : ""}
      />
    </div>
  );
};
